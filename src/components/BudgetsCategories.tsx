...
import { SlidersHorizontal, Info, Edit3, Check, X, Plus, Trash2, Lock, AlertTriangle, Zap, Tag, Sparkles } from "lucide-react";
import { Transaction, AutoCategorizationRule } from "../types";
...
export default function BudgetsCategories({
  ...
  const handleCreateRule = (e: React.FormEvent) => {
    ...
    if (!keyword) return;
    const finalCat = ruleCategory || categories[0] || "Others";
    if (onAddRule) {
      onAddRule(keyword, finalCat);
      setRuleKeyword("");
    }
  };
  ...
  const SimpleProgressBar = ({ month, spent, budgetLimit }: { key?: string, month: string, spent: number, budgetLimit: number }) => {
    ...
    <div className="space-y-8" id="budgets-categories-view">
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-1.5 w-full">
                      ...
                      <button
                        ...
                        className="p-1 text-slate-440 hover:bg-slate-100 rounded cursor-pointer"
                        title="Cancel"
                      >
      ...
      {/* Custom Rules Auto-Categorization Manager */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm" id="custom-rules-card">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5 mb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Define custom rule matches to override or supplement default assignments. Rules run automatically when statements are uploaded.
            </p>
          </div>
          {transactions.length > 0 && (
            <button
              onClick={onApplyRulesToCurrent}
              className="px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-lg text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
              title="Apply these rules to the currently loaded statement"
            >
              <Sparkles className="w-3.5 h-3.5 text-indigo-600 animate-bounce" />
              Apply Rules to Current Ledger
            </button>
          )}
        </div>

        {/* Rule Form & Current Rules List Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          {/* Add Rule Form (1/3 width) */}
          <div className="bg-slate-50/50 border border-slate-150 p-5 rounded-xl">
            <h5 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-4 flex items-center gap-1.5">
            ...
            <form onSubmit={handleCreateRule} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs border border-slate-200 rounded-md p-2.5 focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white transition font-medium"
                  required
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full text-xs border border-slate-200 rounded-md p-2.5 focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white transition font-semibold text-slate-700"
                  required
                >
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-bold shadow-sm transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
          ...
          {/* Active Rules List (2/3 width) */}
          <div className="lg:col-span-2 space-y-4">
            <h5 className="text-xs font-bold uppercase text-slate-500 tracking-wider flex items-center justify-between">
              <span>Active Categorization Rules ({customRules.length})</span>
              <span className="text-[10px] text-slate-400 normal-case font-normal">Rules are evaluated sequentially in order of creation</span>
            </h5>

            {customRules.length === 0 ? (
              <div className="border border-dashed border-slate-200 rounded-xl py-10 px-4 text-center text-xs text-slate-400">
                <Tag className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                No custom categorization rules defined yet. Create one on the left!
              </div>
            ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-150 text-[10px] font-bold uppercase text-slate-400 tracking-wider">
                        ...
                        <th className="py-3 px-4">Auto-Assigned Category</th>
                        ...
                        <tr key={rule.id} className="hover:bg-slate-50/50 transition">
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded transition cursor-pointer inline-flex items-center"
                              title="Delete Rule"
                            >
